...
# Detect the REAL SSH port (same logic as the deploy scripts). The old code
# hardcoded 22 everywhere (UFW limit, GeoIP SSH gate, verify) — on a box with a
# custom SSH port, harden.sh rate-limited/geo-gated the WRONG port and the UFW
# reset dropped the rule that was keeping your session alive. Live listener
# first, sshd_config fallback, 22 as last resort.
detect_ssh_port() {
  local p=""
...
harden_firewall() {
    ...
    info "Allowing: ${SSH_PORT}/tcp (SSH) + ${pub[*]}"
    ...
    if [[ "$OS_FAMILY" == "debian" ]]; then
        ...
        ufw limit "${SSH_PORT}/tcp" comment 'SSH rate limit' >> "$LOGFILE" 2>&1 || true
        for p in "${pub[@]}"; do
            case "$p" in
        ...
        # The 'ssh' service is port 22 only — also open the REAL SSH port when custom.
        [[ "$SSH_PORT" != "22" ]] && firewall-cmd --permanent --add-port="${SSH_PORT}/tcp" >> "$LOGFILE" 2>&1 || true
        for p in "${pub[@]}"; do
    ...
    _log "Firewall configured: ${SSH_PORT}/tcp ${pub[*]}"
}
...
harden_geoip() {
    ...
    if [[ -n "$GEOIP_MYCC" && " $GEOIP_SELECTED " != *" $GEOIP_MYCC "* ]]; then
        ...
        printf "${C_RED}SSH (%s) is geo-gated, so applying this can lock YOU out. Recovery = your provider's web/serial console.${C_RST}\n" "$SSH_PORT"
        if [[ -t 0 ]]; then
            local sure
...
geo_teardown() {
    local p
    for p in "$SSH_PORT" 22; do
        iptables -D INPUT -p tcp --dport "$p" -j "$GATE" 2>/dev/null || true
    done
    ...
    if [[ "$HAVE6" == "1" ]]; then
        for p in "$SSH_PORT" 22; do
            ip6tables -D INPUT -p tcp --dport "$p" -j "$GATE6" 2>/dev/null || true
        done
...
if [[ "$GATE_SSH" == "1" ]]; then
    iptables -C INPUT -p tcp --dport "$SSH_PORT" -j "$GATE" 2>/dev/null || iptables -I INPUT -p tcp --dport "$SSH_PORT" -j "$GATE"
else
    iptables -D INPUT -p tcp --dport "$SSH_PORT" -j "$GATE" 2>/dev/null || true
fi
...
if [[ "$HAVE6" == "1" ]]; then
    ...
    if [[ "$allow6_n" -gt 0 ]]; then
        ...
        if [[ "$GATE_SSH" == "1" ]]; then
            ip6tables -C INPUT -p tcp --dport "$SSH_PORT" -j "$GATE6" 2>/dev/null || ip6tables -I INPUT -p tcp --dport "$SSH_PORT" -j "$GATE6"
        else
            ip6tables -D INPUT -p tcp --dport "$SSH_PORT" -j "$GATE6" 2>/dev/null || true
        fi
...
verify_hardening() {
    ...
    # 2. Firewall
    if [[ "$OS_FAMILY" == "debian" ]]; then
        _check "UFW rate limit SSH"  "ufw status 2>/dev/null | grep -qE '${SSH_PORT}/tcp .*LIMIT'"
