...
# deploy-casaos.sh -- Docker + NPM + CasaOS + Authelia + CrowdSec (v4.3.0-hardened)
# One-click VPS deployment. Usage: sudo ./deploy-casaos.sh
#   Optional env vars:
...
_on_exit() {
  ...
  if [[ "$DEPLOY_STATUS" == "success" ]]; then
    printf "${C_B}  CasaOS:      https://casaos.%s${C_R}\n" "$DOMAIN"
    ...
    printf "${C_B}    casaos (native svc)  ->  127.0.0.1:8080${C_R}\n"
...
access_control:
  ...
  rules:
    - domain: "authelia.${DOMAIN}"
      policy: bypass
    # Wildcard catch-all LAST: any other subdomain (incl. casaos and
...
setup_authelia_snippets() {
  ...
  # which is a 404 -> every request to CasaOS would 500. The correct
...
automate_npm() {
  ...
  # CasaOS: protected by Authelia auth_request. Both snippets are required:
  ...
  local casaos_id=""
  casaos_id=$(npm_create_proxy_host "casaos.${DOMAIN}" "127.0.0.1" 8080 true "$auth_snippet") || true
  [[ -n "$casaos_id" ]] && npm_enable_ssl "$casaos_id" "casaos.${DOMAIN}" || true
...
casaos.${DOMAIN}  ->  127.0.0.1:8080   (systemd service, Authelia 2FA enforced)
...
${C_B}CasaOS${C_R}
  URL:        https://casaos.${DOMAIN}
  Container:  casaos
  Network:    proxy
  Data:       ${DOCKHAND_DATA_DIR}
  Auth:       Authelia 2FA in front + CasaOS built-in SSO/MFA (setup wizard on first visit)
...
${C_B}Containers${C_R}  npm, casaos, authelia, crowdsec
${C_B}Network${C_R}   proxy (bridge)

${C_B}${C_YEL}Done automatically:${C_R}
  - Proxy hosts for CasaOS / Authelia created
  ...
  - Authelia 2FA protecting CasaOS
...
${C_B}Troubleshooting:${C_R}
  Logs:    docker logs -f npm    docker logs -f authelia
