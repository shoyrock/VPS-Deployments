...
readonly SCRIPT_VERSION="2.0.0-casaos"
...
idempotent_cleanup() {
  ...
  rm -rf /var/lib/docker/* /etc/docker/* \
         "${CASAOS_DATA_DIR}" "${CASAOS_CONF_DIR}" "${CASAOS_SHARE_DIR}" "${NPM_DIR}" 2>/dev/null || true
...
main() {
  printf "\n${C_B}${C_CYN}VPS Deployment — Docker + NPM + CasaOS + Fail2Ban${C_R}\n"
