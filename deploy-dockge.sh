#!/usr/bin/env bash
# ═══════════════════════════════════════════════════════════════════════════════
# deploy-dockge.sh — Hardened VPS Deployment (Docker + NPM + Dockge + Fail2Ban)
# v2.0.0-dockge | Usage: chmod +x deploy-dockge.sh && sudo ./deploy-dockge.sh
#
# One-shot, idempotent deployment for fresh VPS instances. Installs Docker CE,
# Nginx Proxy Manager, Dockge, Fail2Ban, and host firewall with the critical
# Docker+UFW compatibility fix applied.
#
...
COMPOSE
  ...
  for i in $(seq 1 40); do
    curl -sf --max-time 5 http://127.0.0.1:5001/ &>/dev/null && { success "Dockge responding"; break; }
    [[ $i -eq 40 ]] && warn "Dockge timed out. May still be starting — check: docker logs dockge"
    sleep 3
  done
  success "Dockge deployed: http://${ip}:5001"
}
...
${C_B}${C_CYN}── SERVICES ──${C_R}
  ...
  ${C_B}Dockge${C_R}
    Direct:    http://${ip}:5001
    Proxy:     Via NPM (add proxy host → http://dockge:5001)
...
${C_B}${C_YEL}── INITIAL SETUP ──${C_R}
  ...
  2. ${C_B}Proxy Hosts:${C_R} In NPM, add:
     - dockge.your-domain.com → http://dockge:5001
