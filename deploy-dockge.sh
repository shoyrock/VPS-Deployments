...
idempotent_cleanup() {
  ...
  mkdir -p "$STACK_DIR" "$NPM_DATA_DIR" "$NPM_LE_DIR" "$NPM_LOGS_DIR" "$CROWDSEC_DIR" "$DOCKGE_DATA_DIR"
  chmod 750 "$STACK_DIR"
...
verify_deployment() {
  ...
  # Containers
  local want="npm dockge authelia crowdsec crowdsec-dashboard"
  local c
  for c in $want; do
...
${C_B}Containers${C_R}  npm, dockge, authelia, crowdsec, crowdsec-dashboard
${C_B}Network${C_R}   proxy (bridge)

${C_B}${C_YEL}Done automatically:${C_R}
  - Proxy hosts for Dockge / Authelia / CrowdSec created
