...
USERS
  ...
  # Append the email line with domain substitution
  echo "    email: admin@${DOMAIN}" >> "${AUTHELIA_CONFIG_DIR}/users.yml"

  cat >> "${AUTHELIA_CONFIG_DIR}/users.yml" << 'USERS2'
    groups:
      - admins
      - users
USERS2
  ...
  warn "Default admin password is 'authelia' -- CHANGE IT IMMEDIATELY after first login"
}
