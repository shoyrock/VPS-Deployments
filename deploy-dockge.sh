...
# deploy-dockge.sh -- Docker + NPM + Dockge + Authelia + CrowdSec (v4.3.0-hardened)
# One-click VPS deployment. Usage: sudo ./deploy-dockge.sh
#   Optional env vars:
...
readonly DOCKGE_DATA_DIR="${STACK_DIR}/dockge-data"
...
_on_exit() {
  ...
  if [[ "$DEPLOY_STATUS" == "success" ]]; then
    printf "${C_B}  Dockge:      https://dockge.%s${C_R}\n" "$DOMAIN"
    ...
    printf "${C_B}    dockge               ->  port 5001${C_R}\n"
...
setup_dockge() {
  step "Dockge (standalone)"
  mkdir -p "${DOCKGE_DATA_DIR}" "${STACK_DIR}/stacks"
...
COMPOSE_DOCKGE
  ...
  info "Pulling Dockge image..."
  docker compose -p dockge -f "${STACK_DIR}/docker-compose.dockge.yml" pull
  docker compose -p dockge -f "${STACK_DIR}/docker-compose.dockge.yml" up -d

  info "Waiting for Dockge to be ready..."
  for i in $(seq 1 30); do
    printf "\r  ${C_DIM}Waiting for Dockge... %d/30${C_R}" "$i" >&2
    sleep 2
    if docker ps --format '{{.Names}}' | grep -qx "dockge"; then
      printf "\r" >&2
      success "Dockge ready"
      break
    fi
    [[ $i -eq 30 ]] && { printf "\r" >&2; warn "Dockge may still be starting. Check: docker logs dockge"; }
  done
  printf "\r" >&2
...
access_control:
  ...
  rules:
    - domain: "authelia.${DOMAIN}"
      policy: bypass
    # Wildcard catch-all LAST: any other subdomain (incl. dockge and
...
setup_authelia_snippets() {
  ...
  # which is a 404 -> every request to Dockge would 500. The correct
...
register_dockge_stacks() {
  ...
  info "Creating combined compose files in Dockge data directory..."
  ...
  } > "${DOCKGE_DATA_DIR}/infrastructure.yml" 2>/dev/null || true

  # Also copy individual compose files for reference
  cp "${STACK_DIR}/docker-compose.npm.yml" "${DOCKGE_DATA_DIR}/npm.yml" 2>/dev/null || true
  cp "${STACK_DIR}/docker-compose.authelia.yml" "${DOCKGE_DATA_DIR}/authelia.yml" 2>/dev/null || true
  cp "${STACK_DIR}/docker-compose.crowdsec.yml" "${DOCKGE_DATA_DIR}/crowdsec.yml" 2>/dev/null || true
...
automate_npm() {
  ...
  local dockge_id=""
  dockge_id=$(npm_create_proxy_host "dockge.${DOMAIN}" "dockge" 5001 true "$auth_snippet") || true
  [[ -n "$dockge_id" ]] && npm_enable_ssl "$dockge_id" "dockge.${DOMAIN}" || true
...
verify_deployment() {
  ...
  # Containers
  local want=(npm dockge authelia crowdsec)
...
dockge.${DOMAIN}  ->  dockge:5001   (Authelia 2FA enforced)
...
${C_B}Dockge${C_R}
  ...
  Container:  dockge
  Network:    proxy
  Data:       ${DOCKGE_DATA_DIR}
  Auth:       Authelia 2FA in front + Dockge built-in SSO/MFA (setup wizard on first visit)
...
${C_B}Containers${C_R}  npm, dockge, authelia, crowdsec
${C_B}Network${C_R}   proxy (bridge)

${C_B}${C_YEL}Done automatically:${C_R}
  - Proxy hosts for Dockge / Authelia created
  ...
  - Authelia 2FA protecting Dockge
