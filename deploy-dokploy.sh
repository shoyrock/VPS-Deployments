...
# deploy-dokploy.sh -- Docker + NPM + Dokploy + Authelia + CrowdSec (v4.3.0-hardened)
# One-click VPS deployment. Usage: sudo ./deploy-dokploy.sh
#   Optional env vars:
...
_on_exit() {
  ...
  if [[ "$DEPLOY_STATUS" == "success" ]]; then
    printf "${C_B}  Dokploy:      https://dokploy.%s${C_R}\n" "$DOMAIN"
    ...
    printf "${C_B}    dokploy              ->  port 3000${C_R}\n"
...
access_control:
  ...
  rules:
    - domain: "authelia.${DOMAIN}"
      policy: bypass
    # Wildcard catch-all LAST: any other subdomain (incl. dokploy and
...
session:
  ...
  cookies:
    - domain: "${DOMAIN}"
      ...
      default_redirection_url: "https://dokploy.${DOMAIN}"
regulation:
  max_retries: 5
  find_time: 2m
  ban_time: 10m
...
setup_authelia_snippets() {
  ...
  # which is a 404 -> every request to Dokploy would 500. The correct
...
automate_npm() {
  ...
  # Dokploy: protected by Authelia auth_request. Both snippets are required:
  ...
  local dokploy_id=""
  dokploy_id=$(npm_create_proxy_host "dokploy.${DOMAIN}" "dokploy" 3000 true "$auth_snippet") || true
  [[ -n "$dokploy_id" ]] && npm_enable_ssl "$dokploy_id" "dokploy.${DOMAIN}" || true
...
dokploy.${DOMAIN}  ->  dokploy:3000   (Authelia 2FA enforced)
...
${C_B}Dokploy${C_R}
  ...
  Auth:       Authelia 2FA in front + Dokploy built-in SSO/MFA (setup wizard on first visit)
...
${C_B}Containers${C_R}  npm, dokploy, authelia, crowdsec
${C_B}Network${C_R}   proxy (bridge)

${C_B}${C_YEL}Done automatically:${C_R}
  - Proxy hosts for Dokploy / Authelia created
  ...
  - Authelia 2FA protecting Dokploy
...
${C_B}Troubleshooting:${C_R}
  Logs:    docker logs -f npm    docker logs -f dokploy    docker logs -f authelia
