...
verify_deployment() {
  ...
  # Containers
  local want="npm dokploy authelia crowdsec crowdsec-dashboard"
  local c
  for c in $want; do
...
${C_B}Containers${C_R}  npm, dokploy, authelia, crowdsec, crowdsec-dashboard
${C_B}Network${C_R}   proxy (bridge)

${C_B}${C_YEL}Done automatically:${C_R}
  - Proxy hosts for Dokploy / Authelia / CrowdSec created
