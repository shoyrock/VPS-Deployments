...
# deploy-freedombox.sh -- Docker + NPM + FreedomBox + Authelia + CrowdSec (v4.3.0-hardened)
# One-click VPS deployment. Usage: sudo ./deploy-freedombox.sh
#   Optional env vars:
...
readonly STACK_DIR="/opt/freedombox-stack"
...
_on_exit() {
  ...
  if [[ "$DEPLOY_STATUS" == "success" ]]; then
    ...
    printf "${C_B}    freedombox (native)  ->  127.0.0.1:443${C_R}\n"
...
session:
  ...
  cookies:
    - domain: "${DOMAIN}"
      ...
      default_redirection_url: "https://freedombox.${DOMAIN}"
regulation:
  max_retries: 5
  find_time: 2m
  ban_time: 10m
...
setup_authelia_snippets() {
  ...
  # which is a 404 -> every request to FreedomBox would 500. The correct
...
automate_npm() {
  ...
  # FreedomBox: protected by Authelia auth_request. Both snippets are required:
  ...
  local freedombox_id=""
  freedombox_id=$(npm_create_proxy_host "freedombox.${DOMAIN}" "127.0.0.1" 443 true "$auth_snippet") || true
  [[ -n "$freedombox_id" ]] && npm_enable_ssl "$freedombox_id" "freedombox.${DOMAIN}" || true
...
${C_B}FreedomBox${C_R}
  URL:        https://freedombox.${DOMAIN}
  Container:  freedombox
  Network:    proxy
  Data:       ${DOCKHAND_DATA_DIR}
  Auth:       Authelia 2FA in front + FreedomBox built-in SSO/MFA (setup wizard on first visit)
...
harden_ssh() {
  ...
  # Fully disable root login ONLY when it cannot lock the operator out: the deploy
  ...
  [[ -n "${SUDO_USER:-}" && "$SUDO_USER" != "root" && -s "/home/${SUDO_USER}/.ssh/authorized_keys" ]] && root_login="no"
...
verify_installation() {
  ...
  ss -tlnp 2>/dev/null | grep -q ':80'  && info "  Port 80 (HTTP):      ${C_GRN}listening${C_R}" || warn "  Port 80 (HTTP):      not listening"
  ss -tlnp 2>/dev/null | grep -q ':443' && info "  Port 443 (HTTPS):    ${C_GRN}listening${C_R}" || warn "  Port 443 (HTTPS):    not listening"
  ss -tlnp 2>/dev/null | grep -q ':9090' && info "  Port 9090 (Cockpit): ${C_GRN}listening${C_R}" || warn "  Port 9090 (Cockpit): not listening"
...
main() {
  printf "\n${C_B}${C_CYN}VPS Deployment -- Docker + NPM + FreedomBox + Authelia + CrowdSec${C_R}\n" >&2
