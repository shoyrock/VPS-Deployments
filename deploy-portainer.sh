...
_on_exit() {
  ...
  if [[ "$DEPLOY_STATUS" == "success" ]]; then
    ...
    printf "${C_B}    portainer            ->  port 9000${C_R}\n"
...
COMPOSE_PORTAINER
  ...
  docker compose -p portainer -f "${STACK_DIR}/docker-compose.portainer.yml" pull
  docker compose -p portainer -f "${STACK_DIR}/docker-compose.portainer.yml" up -d
...
verify_deployment() {
  ...
  # Containers
  local want="npm portainer authelia crowdsec"
  local c
  for c in $want; do
...
${C_B}${C_YEL}Done automatically:${C_R}
  - Proxy hosts for Portainer / Authelia created
