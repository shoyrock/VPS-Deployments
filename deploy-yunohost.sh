...
# deploy-yunohost.sh -- Docker + NPM + YunoHost + Authelia + CrowdSec (v4.3.0-hardened)
# One-click VPS deployment. Usage: sudo ./deploy-yunohost.sh
#   Optional env vars:
...
readonly STACK_DIR="/opt/yunohost-stack"
...
access_control:
  ...
  rules:
    ...
    # Wildcard catch-all LAST: any other subdomain (incl. yunohost and
...
session:
  ...
  cookies:
    - domain: "${DOMAIN}"
      ...
      default_redirection_url: "https://yunohost.${DOMAIN}"
regulation:
  max_retries: 5
  find_time: 2m
  ban_time: 10m
...
setup_authelia_snippets() {
  ...
  # which is a 404 -> every request to YunoHost would 500. The correct
...
automate_npm() {
  ...
  # YunoHost: protected by Authelia auth_request. Both snippets are required:
  ...
  local yunohost_id=""
  yunohost_id=$(npm_create_proxy_host "yunohost.${DOMAIN}" "127.0.0.1" 443 true "$auth_snippet") || true
  [[ -n "$yunohost_id" ]] && npm_enable_ssl "$yunohost_id" "yunohost.${DOMAIN}" || true
...
yunohost.${DOMAIN}  ->  127.0.0.1:443   (YunoHost web admin, manual postinstall required)
...
${C_B}YunoHost${C_R}
  URL:        https://yunohost.${DOMAIN}
  Container:  yunohost
  Network:    proxy
  Data:       ${DOCKHAND_DATA_DIR}
  Auth:       Authelia 2FA in front + YunoHost built-in SSO/MFA (setup wizard on first visit)
...
${C_B}Containers${C_R}  npm, yunohost, authelia, crowdsec, crowdsec-dashboard
${C_B}Network${C_R}   proxy (bridge)

${C_B}${C_YEL}Done automatically:${C_R}
  - Proxy hosts for YunoHost / Authelia / CrowdSec created
  ...
  - Authelia 2FA protecting YunoHost
...
${C_B}Troubleshooting:${C_R}
  Logs:    journalctl -u yunohost -f    docker logs -f npm    docker logs -f authelia
...
main() {
  printf "\n${C_B}${C_CYN}VPS Deployment -- Docker + NPM + YunoHost + Authelia + CrowdSec${C_R}\n" >&2
