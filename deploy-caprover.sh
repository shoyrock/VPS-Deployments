#!/usr/bin/env bash
# deploy-caprover.sh — Hardened VPS Deployment (Docker Swarm + CapRover + NPM + Fail2Ban)
...
readonly SCRIPT_VERSION="2.0.0-caprover"
readonly SCRIPT_NAME="deploy-caprover.sh"
...
idempotent_cleanup() {
  ...
  info "Cleaning up CapRover/Swarm state..."
  docker swarm leave --force 2>/dev/null || true
  rm -rf /captain 2>/dev/null || true
...
setup_firewall_debian() {
  ...
  ufw allow 4789/udp comment 'Docker Swarm VXLAN'
...
${C_B}${C_CYN}-- SERVICES --${C_R}
  ...
  ${C_B}Docker${C_R}
    ...
    Swarm:     $(docker info --format '{{.Swarm.LocalNodeState}}' 2>/dev/null || echo unknown)
...
UFWCMD
  ...
  cat << 'FIRECMD'
     firewall-cmd --permanent --remove-port=3000/tcp --remove-port=81/tcp
     firewall-cmd --reload
FIRECMD
fi)
...
${C_B}${C_CYN}-- TROUBLESHOOTING --${C_R}
  ...
  Logs:       docker logs -f caprover    docker logs -f npm
  Restart:    docker restart caprover    cd ${NPM_DIR} && docker compose restart
...
main() {
  printf "\n${C_B}${C_CYN}  VPS Deployment — Docker Swarm + CapRover + NPM + Fail2Ban${C_R}\n"
