...
# deploy-runtipi.sh -- Docker + NPM + Runtipi + Authelia + CrowdSec (v4.3.0-hardened)
# One-click VPS deployment. Usage: sudo ./deploy-runtipi.sh
#   Optional env vars:
...
_on_exit() {
  ...
  if [[ "$DEPLOY_STATUS" == "success" ]]; then
    printf "${C_B}  Runtipi:      https://runtipi.%s${C_R}\n" "$DOMAIN"
...
access_control:
  ...
  rules:
    ...
    # Wildcard catch-all LAST: any other subdomain (incl. runtipi and
...
setup_authelia_snippets() {
  ...
  # which is a 404 -> every request to Runtipi would 500. The correct
...
automate_npm() {
  ...
  # Runtipi: protected by Authelia auth_request. Both snippets are required:
  ...
  local runtipi_id=""
  runtipi_id=$(npm_create_proxy_host "runtipi.${DOMAIN}" "tipi-reverse-proxy" 80 true "$auth_snippet") || true
  [[ -n "$runtipi_id" ]] && npm_enable_ssl "$runtipi_id" "runtipi.${DOMAIN}" || true
...
verify_deployment() {
  ...
  # Containers
  local want="npm tipi-reverse-proxy authelia crowdsec crowdsec-dashboard"
  local c
  for c in $want; do
...
runtipi.${DOMAIN}  ->  tipi-reverse-proxy:80   (Authelia 2FA enforced)
...
${C_B}Runtipi${C_R}
  URL:        https://runtipi.${DOMAIN}
  Container:  runtipi
  Network:    proxy
  Data:       ${DOCKHAND_DATA_DIR}
  Auth:       Authelia 2FA in front + Runtipi built-in SSO/MFA (setup wizard on first visit)
...
${C_B}Containers${C_R}  npm, runtipi, authelia, crowdsec, crowdsec-dashboard
${C_B}Network${C_R}   proxy (bridge)

${C_B}${C_YEL}Done automatically:${C_R}
  - Proxy hosts for Runtipi / Authelia / CrowdSec created
  ...
  - Authelia 2FA protecting Runtipi
...
main() {
  printf "\n${C_B}${C_CYN}VPS Deployment -- Docker + NPM + Runtipi + Authelia + CrowdSec${C_R}\n" >&2
