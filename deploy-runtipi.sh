...
_on_exit() {
  ...
  if [[ "$DEPLOY_STATUS" == "success" ]]; then
    ...
    printf "${C_B}    tipi-reverse-proxy   ->  port 80${C_R}\n"
...
verify_deployment() {
  ...
  # Containers
  local want="npm tipi-reverse-proxy authelia crowdsec"
  local c
  for c in $want; do
...
${C_B}Containers${C_R}  npm, runtipi, authelia, crowdsec
${C_B}Network${C_R}   proxy (bridge)

${C_B}${C_YEL}Done automatically:${C_R}
  - Proxy hosts for Runtipi / Authelia created
