...
_on_exit() {
  ...
  if [[ "$DEPLOY_STATUS" == "success" ]]; then
    ...
    printf "${C_B}    coolify              ->  port 8000${C_R}\n"
...
verify_deployment() {
  ...
  # Containers
  local want="npm coolify authelia crowdsec"
  local c
  for c in $want; do
...
${C_B}Containers${C_R}  npm, coolify, authelia, crowdsec
${C_B}Network${C_R}   proxy (bridge)

${C_B}${C_YEL}Done automatically:${C_R}
  - Proxy hosts for Coolify / Authelia created
