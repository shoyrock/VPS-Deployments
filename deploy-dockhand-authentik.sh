...
bootstrap_authentik() {
  ...
  if [[ -z "$flow_pk" ]]; then
    warn "Could not resolve Authentik authorization flow via API (blueprints not applied after ~2 min). Create the Dockhand provider manually."
    return 0
  fi
  ...
  # 4. Provision Dockhand AND the CrowdSec dashboard. Gating CrowdSec from first boot
  ...
  local prov_pks=() _p
  _p=$(_ak_provision "dockhand" "dockhand" "Dockhand" "http://dockhand:3000" "https://dockhand.${DOMAIN}") && [[ -n "$_p" ]] && prov_pks+=("$_p")
  ...
  if _ak_api "/outposts/instances/${outpost_pk}/" -X PATCH \
    ...
    success "Providers attached to embedded outpost + authentik_host set (dockhand$([[ ${#prov_pks[@]} -gt 1 ]] && printf ' + crowdsec'))"
  else
...
automate_npm() {
  ...
  local dockhand_id=""
  # ORDER MATTERS: create Dockhand WITHOUT the auth gate, get its cert via HTTP-01
  ...
  [[ -n "$dockhand_id" ]] && npm_apply_auth_gate "$dockhand_id" "$auth_snippet" || true
...
COMPOSE_WEBUI
  ...
  if [[ -n "${NPM_TOKEN:-}" ]]; then
    ...
    # like Dockhand. Its first visit creates the dashboard's OWN admin account, so an
