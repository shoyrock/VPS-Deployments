...
verify_deployment() {
  ...
  local want=(npm arcane authentik-server authentik-worker authentik-postgres authentik-redis crowdsec)
...
hdr "Containers"
for c in npm arcane authentik-server authentik-worker authentik-postgres authentik-redis crowdsec; do
...
${C_B}Containers${C_R}  npm, arcane, authentik-server/worker/postgres/redis, crowdsec
