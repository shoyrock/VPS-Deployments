...
# deploy-dockhand.sh -- Docker + NPM + Dockhand + Authelia + CrowdSec (v4.3.0-hardened)
# One-click VPS deployment. Usage: sudo ./deploy-dockhand.sh
#   Optional env vars:
#     FORCE_CLEANUP=1        skip the destructive-cleanup confirmation
#     CROWDSEC_CHOICE=fail2ban   use Fail2Ban on the host instead of CrowdSec
set -euo pipefail
IFS=$'\n\t'

readonly SCRIPT_VERSION="4.3.0-hardened"
...
CROWDSEC_CHOICE="${CROWDSEC_CHOICE:-crowdsec}"   # crowdsec | fail2ban
METABASE_PASS=""
...
idempotent_cleanup() {
  ...
  if command -v docker &>/dev/null && [[ -n "$(docker ps -aq 2>/dev/null)" ]]; then
    if [[ "${FORCE_CLEANUP:-0}" != "1" ]]; then
      ...
      [[ "$_confirm" == "YES" ]] || fatal "Aborted by user before destructive cleanup. Re-run with FORCE_CLEANUP=1 to skip this prompt."
    fi
...
setup_fail2ban() {
  step "Fail2Ban (host-level alternative to CrowdSec)"
...
JAIL
  ...
  success "Fail2Ban installed with sshd + NPM jails"
}

setup_crowdsec() {
  if [[ "$CROWDSEC_CHOICE" == "fail2ban" ]]; then
    setup_fail2ban
    return 0
  fi
...
BOUNCER
    ...
    if systemctl is-active --quiet crowdsec-firewall-bouncer; then
      success "Firewall bouncer registered and running"
    else
      warn "Firewall bouncer installed but not active. Check: journalctl -u crowdsec-firewall-bouncer -n 50"
    fi
  else
...
dockhand.${DOMAIN}  ->  dockhand:3000   (Authelia 2FA enforced)
authelia.${DOMAIN}  ->  authelia:9091
$(if [[ "$CROWDSEC_CHOICE" == "crowdsec" ]]; then echo "crowdsec.${DOMAIN}  ->  crowdsec-dashboard:3000"; else echo "# Fail2Ban active on host - no proxy host needed"; fi)
...
${C_B}Dockhand${C_R}
  ...
  Auth:       Authelia 2FA in front + Dockhand built-in SSO/MFA (setup wizard on first visit)
...
$(if [[ "$CROWDSEC_CHOICE" == "crowdsec" ]]; then cat << CS
...
${C_B}Fail2Ban${C_R}
  Status:  fail2ban-client status nginx-proxy-manager
  Ban:     fail2ban-client set nginx-proxy-manager banip <IP>
  Unban:   fail2ban-client set nginx-proxy-manager unbanip <IP>
F2B
fi)
...
${C_B}Containers${C_R}  npm, dockhand, authelia$(if [[ "$CROWDSEC_CHOICE" == "crowdsec" ]]; then echo ", crowdsec, crowdsec-dashboard"; fi)
${C_B}Network${C_R}   proxy (bridge)

${C_B}${C_YEL}Done automatically:${C_R}
  - Proxy hosts for Dockhand / Authelia$(if [[ "$CROWDSEC_CHOICE" == "crowdsec" ]]; then echo " / CrowdSec"; fi) created
  ...
  - Authelia 2FA protecting Dockhand
...
${C_B}Troubleshooting:${C_R}
  Logs:    docker logs -f npm    docker logs -f dockhand    docker logs -f authelia
